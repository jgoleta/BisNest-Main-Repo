document.addEventListener("DOMContentLoaded", () => {
  ...
  const users = [
    {
      ...
      perms: { add: true, edit: true, del: false },
    },
    {
      id: "melissa",
      name: "Melissa",
      email: "melissa@example.com",
      perms: { add: false, edit: true, del: false },
    },
    {
      id: "guest",
      name: "Guest",
      email: "guest@example.com",
      perms: { add: false, edit: false, del: false },
    },
  ];
  ...
  function showPermissionsModal(index) {
    ...
    backdrop.innerHTML = `
                    <div class="modal" role="dialog" aria-modal="true" aria-label="Manage permissions for ${
                      user.name
                    }">
                        <h3>Manage permissions</h3>
                        <div class="small">User: <strong>${
                          user.name
                        }</strong> — ${user.email}</div>
                        <form class="perm-form">
                            <label><input type="checkbox" name="add" ${
                              user.perms.add ? "checked" : ""
                            }> Allow Add</label>
                            <label><input type="checkbox" name="edit" ${
                              user.perms.edit ? "checked" : ""
                            }> Allow Edit</label>
                            <label><input type="checkbox" name="del" ${
                              user.perms.del ? "checked" : ""
                            }> Allow Delete</label>
                        </form>
                        <div class="row">
